...
class DDPMNoiseShiftScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def __init__(
        ...
        # Precompute eta and gamma via separate helper function
        self.eta, self.gamma, self.tau = self._compute_coefficients()
    ...
    def _compute_coefficients(self):
        # Compute eta and gamma for each timestep based on self.alphas_cumprod.
        num_timesteps = self.alphas_cumprod.shape[0]
        eta = torch.zeros(num_timesteps, dtype=self.alphas_cumprod.dtype, device=self.alphas_cumprod.device)
        gamma = torch.zeros(num_timesteps, dtype=self.alphas_cumprod.dtype, device=self.alphas_cumprod.device)
        tau = torch.zeros(num_timesteps, dtype=self.alphas_cumprod.dtype, device=self.alphas_cumprod.device)
        for t in range(num_timesteps):
            alpha_prod_t = self.alphas_cumprod[t]
            alpha_prod_t_prev = self.alphas_cumprod[t-1] if t-1 >= 0 else self.one
            ...
            eta[t] = (sqrt_alpha_prod_t_prev * beta_t) / beta_prod_t
            gamma[t] = (sqrt_alpha_t * beta_prod_t_prev) / beta_prod_t
            tau[t] = (1 - sqrt_alpha_prod_t_prev) * beta_t / beta_prod_t
        return eta, gamma, tau
    ...
    def step(
        ...
        mean_shift_forward: torch.Tensor,
        mean_shift_reverse: torch.Tensor,
        generator=None,
        ...
        if isinstance(prior_labels, int):
            prior_labels = torch.full((B,), prior_labels, dtype=torch.long, device=sample.device)
        ...
        if (prior_labels < 0).any() or (prior_labels >= self.num_prior).any():
            raise ValueError("Each prior label must be in the range [0, H*W - 1].")
        # --------------------------------------------------------------------------------
        ...
        mean_shift_forward = mean_shift_forward.to(prior_labels.device)
        mean_shift_forward = mean_shift_forward[prior_labels] # [10, 3072, 1000] -> [B, 3072, 1000]
        mean_shift_forward_t = mean_shift_forward[..., t] # [B, 3072]
        mean_shift_forward_t = mean_shift_forward_t.view(B, C, H, W) # [B,3,32,32]
        
        pred_original_sample = (sample - model_output) / alpha_prod_t ** (0.5)
        ...
        # Mean Shift from Shifted-Noise Framework
        mean_shift_reverse = mean_shift_reverse.to(prior_labels.device) # [10, 3072, 1000]
        mean_shift_reverse = mean_shift_reverse[prior_labels] # [B, 3072, 1000]
        mean_shift_reverse_t = mean_shift_reverse[..., t] # [B, 3072]
        mean_shift_reverse_t = mean_shift_reverse_t.view(B, C, H, W) # [B, 3, 32, 32]
        
        pred_prev_sample = pred_prev_sample + mean_shift_reverse_t
    ...
    def add_noise(
        ...
        # == Additional operation from Shifted-Noise Framework using precomputed mean shift ==
        ...
        # [B, 3072] -> [B, C, H, W]
        mean_shift_t = mean_shift_t.view(B, C, H, W)

        # Add the mean shift only at the designated spatial element for each sample.
        noisy_samples = noisy_samples + mean_shift_t
        
        shifted_noise = noisy_samples - sqrt_alpha_prod * original_samples

        return noisy_samples, shifted_noise
